...
def calculate_metrics(preds, targets):
    ...
    sum_error = [
        great_circle(
            (pred_lat[i].item(), pred_lon[i].item()),
            (target_lat[i].item(), target_lon[i].item())
        ).kilometers
        for i in range(m)
    ]
    d = sum(sum_error) / len(sum_error)
    ...
    metrics_txt = (
        f"MAE_lat={mae_lat:.4f} | MAE_lon={mae_lon:.4f} | "
        f"RMSE_lat={rmse_lat:.4f} | RMSE_lon={rmse_lon:.4f} | "
        f"MSD={msd:.4f} | D={d:.4f}"
    )
